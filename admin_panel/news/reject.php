...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($article['status'] === 'rejected') {
        $error = 'This article is already rejected.';
    } else {
        $upd = $pdo->prepare("UPDATE news SET status = 'rejected' WHERE id = :id");
        $upd->execute([':id' => $id]);
        $success = 'Article has been rejected.';
