...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($agency['status'] === 'approved' || $agency['status'] === 'active') {
        $error = 'This agency is already approved/active.';
    } else {
        $upd = $pdo->prepare("UPDATE admin_users SET status = 'approved' WHERE id = :id AND role = 'agency'");
        $upd->execute([':id' => $id]);
        $success = 'Agency has been approved successfully.';
...
<div class="container">
    ...
    <?php else: ?>
        ...
        <p>Approving this agency will change their status to <strong>Approved</strong> and grant them access to the panel.</p>
